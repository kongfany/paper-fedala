...
class clientALA(object):
    ...
    def test_metrics(self, model=None):
        ...
        # print(y_prob.shape,y_prob[0])

        y_true = np.concatenate(y_true, axis=0)
        # print(y_true.shape,y_true[0])
        ...
        # 将概率转换为类别标签（每个样本选择概率最高的类别）
        y_pred = np.argmax(y_prob, axis=1)
        y_true1 = np.argmax(y_true, axis=1)



        f1 = 100*metrics.f1_score(y_true1, y_pred, average='macro')


        
        # return test_acc, test_num, auc
        return test_acc, test_num, f1
